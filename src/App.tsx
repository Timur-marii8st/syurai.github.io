...
// Добавили 'arjun_profile'
type PageType = 'home' | 'research' | 'agents' | 'stack' | 'team' | 'timur_profile' | 'arjun_profile' | 'motivi_product';
...
const App: React.FC = () => {
  ...
  if (currentPage !== 'home') {
    return (
        <div className="min-h-screen bg-black font-sans">
            ...
            {/* Кнопка "Back to Home" скрыта на внутренних профилях и продуктах */}
            {currentPage !== 'motivi_product' && currentPage !== 'timur_profile' && currentPage !== 'arjun_profile' && (
              <button 
            ...
            {currentPage === 'agents' && <AgentsPage onNavigateToMotivi={() => setCurrentPage('motivi_product')} />}
            {currentPage === 'stack' && <StackPage />}
            {currentPage === 'team' && <TeamPage 
                onNavigateToTimur={() => setCurrentPage('timur_profile')} 
                onNavigateToArjun={() => setCurrentPage('arjun_profile')} 
            />}
            {currentPage === 'timur_profile' && <TimurProfile onBack={() => setCurrentPage('team')} />}
            {currentPage === 'arjun_profile' && <ArjunProfile onBack={() => setCurrentPage('team')} />} {/* Рендер Арджуна */}
            {currentPage === 'motivi_product' && <MotiviPage onBack={() => setCurrentPage('agents')} />}
        </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-[#EAE0D5] font-sans selection:bg-amber-500/30 overflow-x-hidden relative">
      ...
      {/* ... (Здесь весь код фона без изменений) ... */}
      ...
      <div className="relative z-10 max-w-[90rem] mx-auto px-6 pt-8 pb-20">
        ...
        <header className="flex items-center justify-between mb-20 relative">
          <div 
            onClick={() => setCurrentPage('home')}
            className="text-xl font-bold tracking-tight text-white/90 hover:text-amber-400 transition-colors cursor-pointer uppercase"
          >
            Syurai.ai
          </div>
          ...
          <nav className="bg-[#1a1816]/40 backdrop-blur-md border border-white/10 rounded-full p-1 shadow-[inset_0_1px_10px_rgba(0,0,0,0.2)]">
            <ul className="flex items-center gap-1 text-sm font-medium text-white/70">
              <li><button onClick={() => setCurrentPage('home')} className="px-5 py-2 rounded-full bg-[#2a2620] text-amber-100 shadow-inner border border-amber-900/30 transition-all">Home</button></li>
              <li><button onClick={() => setCurrentPage('research')} className="px-5 py-2 rounded-full hover:bg-white/5 hover:text-white transition-all">Research</button></li>
              <li><button onClick={() => setCurrentPage('agents')} className="px-5 py-2 rounded-full hover:bg-white/5 hover:text-white transition-all">Agents</button></li>
              <li><button onClick={() => setCurrentPage('stack')} className="px-5 py-2 rounded-full hover:bg-white/5 hover:text-white transition-all">Stack</button></li>
              <li><button onClick={() => setCurrentPage('team')} className="px-5 py-2 rounded-full hover:bg-white/5 hover:text-white transition-all">Team</button></li>
            </ul>
          </nav>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl mx-auto">
          {/* ... Research, Agents, Stack Cards (без изменений) ... */}
          
          <div 
            onClick={() => setCurrentPage('research')}
            className="group relative h-[19rem] bg-gradient-to-br from-[#12141a]/60 to-[#0a0c10]/60 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 cursor-pointer overflow-hidden transition-all duration-500 hover:border-amber-500/40 hover:shadow-[0_0_60px_-15px_rgba(217,119,6,0.25)]"
          >
             {/* Content Research */}
             <div className="relative z-10 flex flex-col justify-between h-full">
              <div>
                ...
                <p className="text-lg text-white/70 font-light leading-relaxed max-w-[90%]">
                  How our team expands the horizons of science
                </p>
              </div>
              <div className="flex items-center gap-2 text-sm font-bold text-amber-400/90 group-hover:text-amber-200 transition-colors">
                <span className="border-b-2 border-amber-500/30 pb-1 group-hover:border-amber-300">Read Paper</span>
              </div>
            </div>
          </div>
          ...
          <div 
            ...
            className="group relative h-[19rem] bg-gradient-to-br from-[#12141a]/60 to-[#0a0c10]/60 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-0 cursor-pointer overflow-hidden transition-all duration-500 hover:border-amber-500/40 hover:shadow-[0_0_60px_-15px_rgba(217,119,6,0.25)] flex"
          >
             {/* Content Agents */}
          ...
          <div 
            ...
            className="group relative h-[19rem] bg-gradient-to-br from-[#12141a]/60 to-[#0a0c10]/60 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 overflow-hidden transition-all duration-500 hover:border-amber-500/40 hover:shadow-[0_0_60px_-15px_rgba(217,119,6,0.25)] flex flex-col justify-between cursor-pointer"
          >
          ...
          {/* 4. Team Card (UPDATED VISUALS FOR TWO PEOPLE) */}
          <div 
            onClick={() => setCurrentPage('team')}
            className="group relative h-[19rem] bg-gradient-to-br from-[#12141a]/60 to-[#0a0c10]/60 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 overflow-hidden transition-all duration-500 hover:border-amber-500/40 hover:shadow-[0_0_60px_-15px_rgba(217,119,6,0.25)] flex flex-col cursor-pointer"
          >
